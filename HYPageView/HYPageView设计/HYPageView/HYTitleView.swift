...
protocol HYTitleViewDeleate : class {
    func titleView(_ titleView : HYTitleView, didSelected currentIndex : Int)
}

class HYTitleView: UIView {
    
    // MARK: 定义属性
    weak var delegate : HYTitleViewDeleate?
...
extension HYTitleView : HYContentViewDelegate {
    func contentView(_ contentView: HYContentView, endScroll inIndex: Int) {
        // 1.取出两个Label
        let oldLabel = titleLabels[currentIndex]
